...
const PhotoShareBtn = ({ photo }) => {
  ...
  const shareHandler = async () => {
    setIsLoading(true);
    try {
      await axiosPrivate.post(`/photos/${photo._id}/share`, {
  ...
  const generateLinkHandler = async () => {
    try {
      const res = await axiosPrivate.get(`/photos/${photo._id}/createLink`);
      setTempLink(`http://localhost:3000/photos/temp/${res.data.token}`);
    } catch (error) {
      console.log(
        "🚀 ~ file: PhotoShareBtn.jsx ~ line 93 ~ generateLinkHandler ~ error",
        error
      );
    }
  };
